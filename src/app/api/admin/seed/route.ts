...
export async function POST(request: Request) {
  ...
  if (secret !== (process.env.SEED_SECRET || "physio-seed-2025")) {
  ...
  try {
    ...
    const existing = await prisma.workout.findFirst({
      where: { name: "Full Physio Routine" },
    });
    if (existing) {
      return NextResponse.json({
        message: "Default workout already exists",
        workoutId: existing.id,
      });
    }

    // Find or create exercises by name
    ...
    for (const ex of exercises) {
      ...
      if (!found) {
        found = await prisma.exercise.create({
          data: {
            name: ex.name,
      ...
      createdExercises.push({ ...found, ...ex });
    }
    ...
    for (let i = 0; i < createdExercises.length; i++) {
      ...
      await prisma.workoutExercise.create({
        data: {
          ...
          notes: ex.notes ?? null,
          supersetGroupId:
            "supersetGroupId" in ex
              ? (ex as any).supersetGroupId
              : null,
        },
      });
    }
    ...
    return NextResponse.json({
      message: `Seeded ${createdExercises.length} exercises and workout "${workout.name}"`,
      workoutId: workout.id,
    });
  } catch (error: any) {
